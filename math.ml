(* contain trigonometric and geometric functions *)
...
let rad_to_deg = fun rad ->
  rad *. 180. /. pi;;

let deg_to_rad = fun deg ->
  deg *. pi /. 180.;;
...
let radToVect: float -> vector2D = fun rad ->
  (* retourne un vecteur normé correspondant à l angle en rad *)
  cos rad , sin rad;;

let degToVect: float -> vector2D = fun deg ->
  (* retourne un vecteur normé correspondant à l'angle en deg *)
  cos (deg_to_rad deg), sin (deg_to_rad deg);;
