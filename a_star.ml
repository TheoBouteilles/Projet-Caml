      ...
      begin
        ...
        if not (Memory.already_expanded m u) then
          begin
            ...
            let ls = next u in
            let aux = fun v ->
              if (not (Memory.mem m v))||((Memory.get_cost m v) > ((Memory.get_cost m u) +. (k u v))) then
                let costv = (Memory.get_cost m u) +. (k u v) in
                let fv = costv +. h v in
                let fatherv = u in
                begin
...
module type Astar = sig
  include Model
  val search: user_param -> state list option
end
